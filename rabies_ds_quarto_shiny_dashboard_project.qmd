...
world_deaths <- hdi_rabies_death_joined |> 
  group_by(year) |> 
  summarize(yearly_world_deaths = sum(fact_value_numeric, na.rm = TRUE))
...
output$plot <- renderPlotly({
  rabies_plot <- hdi_rabies_death_joined |>
  filter(country %in% c("China", "India", "Philippines", "Myanmar (Burma)", "Kenya", "Vietnam")) |>
  ggplot(aes(x = year, y = fact_value_numeric, color = country)) +
  geom_line(size = 1) +  
  geom_line(
    data = world_deaths,
    aes(x = year, y = yearly_world_deaths),
    inherit.aes = FALSE,
    color = "black",
    size = 1.2,
    linetype = "dashed"
  ...
  labs(
    ...
    y = "Number of deaths",
    color = "Country",
    title = "Annual Human Rabies Deaths in High-Burden Countries and Worldwide",
    subtitle = "Dashed black line indicates global yearly sum"
  ) +
    theme_minimal()
  
  ggplotly(rabies_plot)
})
```
