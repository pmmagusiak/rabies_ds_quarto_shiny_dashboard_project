...
### Row - Tabsets of plots / data {.tabset}
...
```{r}
#| title: Datatable
datatable(hdi_rabies_death_joined)
```
...
percent_all_deaths <- reactive({
  hdi_rabies_death_joined |>
    filter(year == input$year) |>
    mutate(percent_all_deaths = round(fact_value_numeric / sum(fact_value_numeric) * 100)) |>
    arrange(desc(fact_value_numeric)) |>
    slice_head(n = 1) |> 
